...
vector<HUDBarElement>::iterator f_Bars_BarGetIter(char* s_BarName, int modIdx)
{
	int iFound = 0;
	int i;
...
int f_Bars_BarGetKey(char* s_BarName, int modIdx)
{
	int iFound = 0;
	int i;
	int iIndex;
...
int f_Bars_BarGetKeyAlt(char* s_BarName, int modIdx)
{
	int iFound = 0;
	int i;
	int iIndex;
...
int f_Bars_BarSetSize(char* s_BarName, int iElement, float fWidth, float fHeight, int modIdx)
{
...
bool f_Bars_AddNewElement(char* s_BarName, float fPosX, float fPosY, int iMeterType, int modIdx)
{

	char s_BarNameLocal[0x4000]{};
	sprintf(s_BarNameLocal, "%s", s_BarName); //added because you can't write yo char pointer from another plugin
	...
	if (component)
	{
		...
		//_MESSAGE("8");
		sprintf(s_BarNameLocal, "%s_%d", s_BarName, modIdx);
		component->name.Set(s_BarNameLocal);
		//_MESSAGE("9");
...
int f_Bars_BarSetValueScriptVar(char* s_BarName, ScriptVar* ScriptVariable,int iScriptValueCalculateMax, int modIdx)
{
...
int f_Bars_BarSetValueMax(char* s_BarName, float fValue, int modIdx)
{
...
//1 text below
//2 text above
int f_Bars_BarSetAutoPos(char* s_BarName, int iRequest, int modIdx)
{
	int iKey = f_Bars_BarGetKey(s_BarName, modIdx);
	if (iKey >= 0) {}
	else { return -1; }
...
int f_Bars_BarDestroyEXECUTE(char* s_BarName, int modIdx)
{
	int iKey = f_Bars_BarGetKey(s_BarName, modIdx);
	if (iKey >= 0) {}
	else { return -1; }
...
bool Cmd_HudBarDestroy_Execute(COMMAND_ARGS)
{
	...
	*result = f_Bars_BarDestroyEXECUTE(s_BarName, modIdx);
	return true;
}



int f_Bars_BarSetBarTrait(char* s_BarName, int iTrait, float fValue, int modIdx)
{
...
//2 for Text
//10 For Bar
int f_Bars_BarSetPos(char* s_BarName, int iElement, float fPosX, float fPosY, int modIdx)
{
...
int f_Bars_BarSetVisible(char* s_BarName, int iElement, int iValue, int modIdx)
{
...
//1 For Frame
//3 for ImageEx
int f_Bars_BarSetTexture(char* s_BarName, int iElement, char* s_FileName, int iAutomaticSize, int modIdx)
{
...
//1 for Prefix
//2 for PostFix
int f_Bars_BarSetTextString(char* s_BarName,int iElement, char* s_Name, int modIdx)
{
...
int f_Bars_BarSetMeterTrait(char* s_BarName, int iTrait, float fValue, int modIdx)
{
...
int f_Bars_BarSetValueUDF(char* s_BarName, Script* FunctionToCall, int iScriptValueCalculateMax, TESObjectREFR* FunctionCaller,  int modIdx)
{
...
int f_Bars_BarShowBar(char* s_BarName, int iValue, int modIdx)
{
...
int f_Bars_BarSetAlpha(char* s_BarName, int iTrait, float fValue, int modIdx)
{
...
//3 for ImageEx
//10 for ALL
int f_Bars_BarSetColor(char* s_BarName, int iElement, float fRed, float fGreen, float fBlue, int modIdx)
{
...
//3 for ImageEx
//10 for ALL
int f_Bars_BarSetSystemColor(char* s_BarName, int iTrait, int iValue, int modIdx)
{
...
int f_Bars_SetTextTrait(char* s_BarName, int iElement, int iTrait, float fValue, int modIdx)
{
	int iValue = fValue;
	int iKey = f_Bars_BarGetKey(s_BarName, modIdx);
	if (iKey >= 0) {}
...
int f_Bars_BarSetSizeAlt(char* s_BarName, int iElement, float fWidth, float fHeight, int modIdx)
{
...
int f_Bars_BarSetDepth(char* s_BarName, int iElement, float fValue, int modIdx)
{
	int iRound = fValue;
	int iKey = f_Bars_BarGetKey(s_BarName, modIdx);
	if (iKey >= 0) {}
...
int f_Bars_BarSetZoom(char* s_BarName, int iElement, float fValue, int modIdx)
{
...
int f_Bars_BarSetRotation(char* s_BarName, int iElement,int iTrait, float fValue, int modIdx)
{
...
int f_Bars_BarSetItem(char* s_BarName, TESForm* HookedItem, int modIdx, int iRequest)
{
	int iKey = f_Bars_BarGetKey(s_BarName, modIdx);
	if (iKey >= 0) {}
	else { return -1; }
...
int f_Bars_BarSetFormList(char* s_BarName, BGSListForm* listForm, int modIdx, int iRequest)
{
	int iKey = f_Bars_BarGetKey(s_BarName, modIdx);
	if (iKey >= 0) {}
	else { return -1; }
...
int f_Bars_BarOutputDebugInfo(int iKey, int modIdx)
{
	_MESSAGE("-------");
...
int f_Bars_BarGetDebugInfo(char* s_BarName, int modIdx)
{
	int iKey = f_Bars_BarGetKey(s_BarName, modIdx);
	if (iKey >= 0) {}
	else { return -1; }
...
float f_Bars_BarGetBarTrait(char* s_BarName, int iTrait, int modIdx)
{
...
//3 for ImageEx
//10 for ALL elements.
int f_Bars_BarSetIndent(char* s_BarName, int iElement, float fValue, int modIdx)
{
...
float f_Bars_BarGetVisible(char* s_BarName, int iElement, int modIdx)
{
