...
DEFINE_COMMAND_PLUGIN(SetBobbyPinHealth, "", 0, 1, kParams_Tomm_OneFloatOptional)
DEFINE_COMMAND_PLUGIN(GetLockPickMenuValue, "", 0, 1, kParams_Tomm_OneInt)
DEFINE_COMMAND_ALT_PLUGIN(KillAllAlt,KillAll2, "", 0, 0, NULL)
...
bool Cmd_KillAllAlt_Execute(COMMAND_ARGS)
{
...
bool Cmd_SetBobbyPinHealth_Execute(COMMAND_ARGS)
{
	float fPickHealth = 0;

	if (ExtractArgsEx(EXTRACT_ARGS_EX, &fPickHealth))
	{

		if (g_interfaceManager->GetTopVisibleMenuIDJIP() != 1014)
		{
			*result = -1;
			return true;
		}
		...
		if (NUM_ARGS > 0)
		{
			(*g_lockPickMenu)->pickHealth = fPickHealth;
		}
		else
		{
			*result = (*g_lockPickMenu)->pickHealth;
		}
	}
	return true;
}
...
bool Cmd_GetLockPickMenuValue_Execute(COMMAND_ARGS)
{
	int iValue = 0;

	if (ExtractArgsEx(EXTRACT_ARGS_EX, &iValue))
	{
		...
		if (iValue == 0)
			*result = (*g_lockPickMenu)->pickAngle;
		else if(iValue == 1)
			*result = (*g_lockPickMenu)->lockLevel;
		else if (iValue == 2)
			*result = (*g_lockPickMenu)->skillLevel;
...
//TESFurniture* Furn = NULL;
//
//
//if (!ExtractArgsEx(EXTRACT_ARGS_EX, &Furn)) return true;
//
//Console_Print("unk1 is %d", Furn->unk90);
//Console_Print("unk2 is %d", Furn->unk91);