...
DEFINE_COMMAND_PLUGIN(GetMouseMovement, "", 0, 3, kParams_Tomm_TwoScriptVars_OneInt)
DEFINE_COMMAND_PLUGIN(GetMouseWheelMovement, "", 0, 1, kParams_Tomm_OneInt)
...
bool Cmd_GetMouseMovement_Execute(COMMAND_ARGS)

{
	int iRequest = 0;
	ScriptVar* outX = NULL, * outY = NULL;

	if (ExtractArgsEx(EXTRACT_ARGS_EX, &outX, &outY, &iRequest))
	{
		if (iRequest)
		{
			outX->data.num = g_inputGlobals->lastMouseMovementX;
			outY->data.num = g_inputGlobals->lastMouseMovementY;
		}
		else {
			outX->data.num = g_inputGlobals->currMouseMovementX;
			outY->data.num = g_inputGlobals->currMouseMovementY;
		}
	}
	return true;
}
...
bool Cmd_GetMouseWheelMovement_Execute(COMMAND_ARGS)

{
	...
	if (ExtractArgsEx(EXTRACT_ARGS_EX,  &iRequest))
	{
		if (iRequest)
		{
			*result = g_inputGlobals->lastMouseWheelScroll;

		}
		else {
			*result = g_inputGlobals->currMouseWheelScroll;
