...
DEFINE_COMMAND_PLUGIN(DebugTextSetZoom, "", 0, 2, kParams_Tomm_DebugTextSetZoom)
...
bool f_DebugTextCreateSUPRect()
{
	...
	if (component)
	{
		...
		sprintf(s_ToInject, "%s", "<rect name=\"SUPDebugText\"> <template name=\"SUPNVSETextTemplateDoNotUse\"> <rect name=\"TextTemplate\"> <text name=\"Text\"> <string></string> <font>1</font> <visible>1</visible> <zoom>1</zoom> <systemcolor>1</systemcolor>   </text>   </rect> </template>      </rect>");
		FileStream tempFile;
...
bool Cmd_DebugTextSetZoom_Execute(COMMAND_ARGS)
{
	alignas(16) char s_textName[0x4000]{};
	char s_ToCheck[0x4000]{};

	float fZoom;
	if (ExtractArgsEx(EXTRACT_ARGS_EX, &s_textName, &fZoom))
	{
		sprintf(s_ToCheck, "%s%s/%s", "HUDMainMenu/SUPDebugText/", s_textName, "Text");
		Tile* component = GetTargetComponent(s_ToCheck);
		if (component)
		{
			component->SetFloat(kTileValue_zoom, fZoom);
		}
	}
	return true;
}
