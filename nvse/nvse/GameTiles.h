...
public:
	...
	struct Value
	{
		...
		__forceinline void SetFloat(float fltVal, bool bPropagate = true) ///////// taken from JIP
		{
	...
	__forceinline Tile* ReadXML(const char* xmlPath) ///////// taken from JIP NOT USED
	{
		return ThisCall<Tile*>(0xA01B00, this, xmlPath);
	}
	char* GetComponentFullName(char* resStr);

	__forceinline void SetFloat(UInt32 id, float fltVal, bool bPropagate = true) ///////// taken from JIP from GameTiles
	{
		ThisCall(0xA012D0, this, id, fltVal, bPropagate);
	}

	__forceinline void SetString(UInt32 id, const char* strVal, bool bPropagate = true) ///////// taken from JIP from GameTiles THANK YOU!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	{
		ThisCall(0xA01350, this, id, strVal, bPropagate);
	}

	__forceinline void GradualSetFloat(UInt32 id, float startVal, float endVal, float seconds, UInt32 changeMode = 0)  // From JiP Game Tiles
	{
	...
	void			DumpAlt(void);
	void			DumpArray(void);
	void			SetUIFloatMass(int iTraitID, float fValue, int iTypeFilter);
};
